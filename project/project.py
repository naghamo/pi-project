...
def follow(obj):
    ...
    while True:
        ...
        if follow:
            ...
            BT.send("f"+obj)
        follow_mode=True
        info=str(BT.recv(1024),'utf-8')
        if(info=='cc'):
            follow=True
        elif(info=='p'):
            os.system('omxplayer things/error.wav &')
...
def goto(obj):
    ...
    if(a1=='p'):
        ...
        if not status:
            ...
            return " "
        d=recinfo('d'+obj)
        spAr.ChangeDutyCycle(25)
        spAl.ChangeDutyCycle(25)
        spBr.ChangeDutyCycle(25)
        spBl.ChangeDutyCycle(25)
        drive(True,True,False,False)
